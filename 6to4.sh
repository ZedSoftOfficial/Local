...
handle_six_to_four() {
    ...
    if [ "$six_to_four_choice" -eq 1 ]; then
        ...
        read -p "Enter the port(s) (comma-separated if multiple): " ports
...
# قوانین IPTables برای پورت‌ها
EOF
)

        for port in $(echo $ports | tr "," "\n"); do
            commands+="iptables -t nat -A PREROUTING -p tcp --dport $port -j DNAT --to-destination 10.10.10.2\n"
        done
...
# قوانین IPTables برای سرور ایران
EOF
)

        for port in $(echo $ports | tr "," "\n"); do
            commands+="iptables -t nat -A PREROUTING -p tcp --dport $port -j DNAT --to-destination 10.10.10.1\n"
        done
